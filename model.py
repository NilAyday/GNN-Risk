...
class my_GraphConvolution5(nn.Module):
    ...
    def forward(self, input, adj,x):
        ...
        support = torch.cat((torch.mm(adj.to_dense(), input),torch.mm(adj.to_dense(), x)),1)
        output = torch.mm(support, self.weight)
...
class my_GraphConvolution9(nn.Module):
    ...
    def forward(self, input, adj,x):
        ...
        #support = 0.9*torch.mm(adj.to_dense(), input) + 0.1*input
        support = torch.cat((0.9*torch.mm(adj.to_dense(), input), 0.1*input),1)
        output = torch.mm(support, self.weight)
...
class my_GraphConvolution12(nn.Module):
    def __init__(self, in_features, out_features,nfeat,n, bias=True):
        super(my_GraphConvolution12, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
    ...
    def forward(self, input, adj,x):
        ...
        #support = 0.9*torch.mm(adj.to_dense(), input) + 0.1*input
        #support = torch.cat((0.8*torch.mm(adj.to_dense(), input), 0.1*input,0.1*x),1)
        support = torch.cat((0.8*torch.mm(adj.to_dense(), input), 0.2*adj.to_dense()), 1)
        output = torch.mm(support, self.weight)
...
class my_GraphConvolution14(nn.Module):
    ...
    def __repr__(self):
        return self.__class__.__name__ + ' (' \
               + str(self.in_features) + ' -> ' \
               + str(self.out_features) + ')'
class my_GraphConvolution16(nn.Module):
    def __init__(self, in_features, out_features,nfeat,n, bias=True):
        super(my_GraphConvolution16, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.weight = Parameter(torch.FloatTensor(in_features, out_features))
        if bias:
...
class my_GCN_X_I(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        h= torch.from_numpy(create_identity_columns_matrix(n=x.shape[0], p=x.shape[1])).detach().requires_grad_().to(self.device)
